...
const { generateWhatsAppLink } = require('./utils/whatsappLink');
...
// Add this function to format order details
function formatOrderMessage(orderData) {
    return `🛵 *Novo Pedido #${orderData.pedidoId}*\n\n` +
           ...
           `📍 *Endereço:* ${orderData.endereco || 'Retirada no local'}`;
}

// Add this function to handle order processing
async function processOrderAndGenerateLink(orderData) {
    const message = formatOrderMessage(orderData);
    const whatsappLink = generateWhatsAppLink(orderData.telefone, message);
    
    try {
        // You can also send the message directly using the client
        const chatId = `${orderData.telefone}@c.us`;
        ...
        return {
            success: false,
            whatsappLink, // Still return the link even if direct message fails
            error: error.message
        };
    }
}

// Add new route to handle order processing
app.post('/process-order', async (req, res) => {
    try {
        const result = await processOrderAndGenerateLink(req.body);
        res.json(result);
    } catch (error) {
        res.status(500).json({
            success: false,
